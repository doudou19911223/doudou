...
- 受保护的封装 protected
    - 受保护的封装是对对象成员进行一定级别的封装，然后，在类中或者子类中都可以进行访问，但是在外部不可以
...
- 继承的语法，参见jupyter notebook 基础第12节课-oop
- 继承的特征
    ...
    - 子类如果想扩充父类的方法，可以在定义新方法的同时访问父类成员来进行代码重用， 可以使用 [父类名.父类成员] 
       的格式来调用父类成员，也可以使用super().父类成员的 格式来调用
...
- 构造函数
    ...
    - 如果没有定义，则自动查找父类构造函数
    - 如果子类没定义，父类的构造函数带参数，则构造对象时时的参数应该按父类参数构造
- super
    - super不是关键字，而是一个类
    - super的作用是获取MRO（Method Resolution Order）列表中的第一个类
    - super与父类直接没人会实质性关系，但通过super可以调用到父类
...
- 菱形继承/钻石继承问题
    - 多个子类继承同一个父类，这些子类又被同一个类继承，于是继承关系图形形成一个菱形
...
-  多态和多态性https://www.cnblogs.com/luchuangao/p/6739557.html
...
- 无论哪种修饰符都是为了对成员属性进行相应的控制
    - 类的方式：适合多个类中的多个属性共用的一个描述符
    - property：使用当前类中使用，可以控制一个类中多个属性
    - 属性修饰符：使用于当前类中使用，控制一个类中的一个属性
# 6. 类的内置属性
    - __dict__:以字典的方式显示类的成员组成
    - __doc__:获取类的文档信息
    - __name__:获取所有类的名称，如果在模块中使用，获取模块的名称
...
- 抽象类的使用
    ...
    - 抽象类的主要作用是设定类的标准，以便于开发时候具有统一的规范
